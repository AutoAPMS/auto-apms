...
// =============================================================================
// NodeRegistrationOptions - Parent Field Tests
// =============================================================================

class NodeRegistrationOptionsTest : public ::testing::Test
{
};

TEST_F(NodeRegistrationOptionsTest, EncodeOmitsParentFieldWhenEmpty)
{
  const auto opts = YAML::Load("class_name: pkg::Foo\n").as<NodeRegistrationOptions>();

  const YAML::Node encoded = YAML::Node(opts);

  EXPECT_FALSE(encoded[NodeRegistrationOptions::PARAM_NAME_PARENT]);
  EXPECT_EQ(encoded[NodeRegistrationOptions::PARAM_NAME_CLASS].as<std::string>(), "pkg::Foo");
}

// =============================================================================
// NodeManifest::add() - Parent Field Tests
// =============================================================================

TEST_F(NodeManifestTest, AddWithParentMissingNodeSeparatorThrows)
{
  // No '.' separator between manifest identity and node name — exception thrown during decode
  EXPECT_THROW(
    manifest_->add("TestNode", YAML::Load("parent: some_pkg::some_manifest\n").as<NodeRegistrationOptions>()),
    exceptions::NodeManifestError);
}

TEST_F(NodeManifestTest, AddWithParentEmptyNodeNameThrows)
{
  // '.' present but nothing after it — exception thrown during decode
  EXPECT_THROW(
    manifest_->add("TestNode", YAML::Load("parent: some_pkg::some_manifest.\n").as<NodeRegistrationOptions>()),
    exceptions::NodeManifestError);
}

TEST_F(NodeManifestTest, AddWithParentNonExistentResourceThrows)
{
  // Exception thrown during decode when the referenced manifest resource does not exist
  EXPECT_THROW(
    manifest_->add(
      "TestNode", YAML::Load("parent: nonexistent_pkg::nonexistent_manifest.SomeNode\n").as<NodeRegistrationOptions>()),
    exceptions::NodeManifestError);
}
